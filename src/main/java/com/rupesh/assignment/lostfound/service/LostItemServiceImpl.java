...
/**
 * This implementation will do all the operations realted to the Lost Items.
 * 
 * @author Rupesh
 *
 */
...
public class LostItemServiceImpl implements LostItemService {
	...
	/**
	 * This service will process each record from the CSV and store it in LostItem
	 * entity.
	 */
	@Override
		...
		try (BufferedReader reader = new BufferedReader(
			...
			List<LostItem> lostItems = csvParser.getRecords().stream().map(csvRecord -> {
				...
				lostItem.setDateFound(LocalDate.parse(csvRecord.get(3),
						DateTimeFormatter.ofPattern(LostandFoundConstants.DATE_FORMAT_DMY))); // Column 3: Date Found
				return lostItem;
