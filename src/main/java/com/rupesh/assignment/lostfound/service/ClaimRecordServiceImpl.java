...
public class ClaimRecordServiceImpl implements ClaimRecordService {
	...
	private final LnFUserRepository lostItemUserRepository;

	@Autowired
	public ClaimRecordServiceImpl(ClaimRecordRepository claimRecordRepository, LostItemRepository lostItemRepository,
			LnFUserRepository lostItemUserRepository) {
	...
	/**
	 * Once the claims the lost item, the quantity will be validated; the customer
	 * ID and the item ID will also be validated and will give an exception to user
	 * if the user or item is not found. In case the quantity claimed by user is
	 * more than the found items, user will an exception. Once all the validations
	 * are done, a insert record will be sent to the claim record entity; parallely,
	 * it will also set the isClaied to True in Lost Item entity.
	 * 
	 * In case the item is already claimed by the same customer, the user will get
	 * an error, and if another customer is trying to claim, the user will get a
	 * warning but the details will be stored.
	 */
	...
	public String claimLostItem(Long userId, String itemId, int quantity) {
		...
		if (lostItem.getQuantity() < quantity)
			return LostandFoundConstants.QUANTITY_LESS_THAN_REQUESTED;
		...
		lostItem.setClaimed(lostItem.getQuantity() == 0);

		lostItemRepository.save(lostItem);

		if (!claimRecordRepository.findByLostItem(lostItem).isEmpty())
			return LostandFoundConstants.ITEM_ALREADY_CLAIMED + LostandFoundConstants.CLAIMED_SUCCESS + user.getName()
					+ ".";
